#!/usr/bin/env python3
"""Run fail-closed acceptance for NATIVE_RESEARCH_METRICS_READINESS_001."""
...
def environment() -> dict[str, str]:
    return {
        **os.environ,
        ...
        "PYTHONPYCACHEPREFIX": "/tmp/native-research-metrics-acceptance-pyc",
    ...
    for path in sorted((ROOT / "tests").rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for decorator in node.decorator_list:
                ...
                if name.rsplit(".", 1)[-1] in prohibited:
...
def historical_inventory_check() -> dict[str, Any]:
    evidence = ROOT / "evidence/repair/native-research-metrics-readiness-001"
    inventory = json.loads((evidence / "historical-integrity.json").read_text(encoding="utf-8"))
    historical = ROOT / inventory["historical_epoch"]
    observed = {
    ...
    passed = (
        ...
        and inventory["historical_files_modified"] is False
        and inventory["files"] == observed
        and inventory["inventory_identity"]
        == hashlib.sha256(
            json.dumps(
                observed,
                sort_keys=True,
...
def main() -> int:
    ...
    historical = historical_inventory_check()
    ...
    gates = {
        ...
        **{f"tests_{name}": value["status"] for name, value in test_runs.items()},
        ...
        "historical_evidence_integrity": historical["status"],
    ...
    result = {
        "schema": "native-research-metrics-readiness-acceptance-v1",
        ...
        "commands": {
            name: {
                key: value[key]
                for key in ("label", "command", "returncode", "status")
            }
    ...
    logs = (
        ...
        ("NATIVE METRICS TARGETED", targeted),
